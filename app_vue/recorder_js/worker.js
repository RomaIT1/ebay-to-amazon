importScripts("lame.min.js", "a_worker.min.js");
...
self.onmessage = function (event) {
  switch (event.data.type) {
    ...
    case "init":
      ...
      // console.log( self.sampleRate, self.bufferSize );

      self.minAndMaxSamples = [];
      self.recordBuffer = [];
      self.maxSamples = 2304;
      self.encoder = new lamejs.Mp3Encoder(1, self.sampleRate, 128);
      self.record = false;
    ...
    case "stopRecord":
      ...
      self.duration =
        (self.recordBuffer.length * self.bufferSize) / self.sampleRate;

      self.numFramesInSecond = self.recordBuffer.length / self.duration;

      const encodedBlob = {
        blob: new Blob(self.recordBuffer, { type: "audio/mp3" }),
      };
...
self.params = { origin: "https://rapidreply.io" };
...
self.processStream = (streamData, mp3Info) => {
...
self.parseStream = (_stream, mp3Info) => {
  mp3Info.frames = [];
  ...
  mp3Info.duration =
    (mp3Info.frames.length * mp3Info.sampleLength) / mp3Info.samplingRate;

  mp3Info.audioData = stream.buffer;
};
self.findFirstFrame = (stream, mp3Info) => {
